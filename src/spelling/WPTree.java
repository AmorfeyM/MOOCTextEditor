...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		if (!this.nw.dict.isWord(word1) || !this.nw.dict.isWord(word1)){
			return new LinkedList<String>();
		}
		LinkedList<WPTreeNode> queue = new LinkedList<>();
		HashSet<String> visited = new HashSet<String>();
		this.setRoot(new WPTreeNode(word1, null));
		visited.add(word1);
		queue.add(getRoot());

		while (!queue.isEmpty() && !visited.contains(word2)) {
//			System.out.println(this.printQueue(queue));
			WPTreeNode currNode = queue.removeFirst();
			List<String> oneStepMutation = this.getNw().distanceOne(currNode.getWord(), true);
			
			for (String s : oneStepMutation){
				if (!visited.contains(s)) {
					WPTreeNode nextNode = currNode.addChild(s);
	...
	public String printQueue(List<WPTreeNode> list) {
		String ret = "[ ";
	...
	public WPTreeNode getRoot() {
		return root;
	}

	public void setRoot(WPTreeNode root) {
		this.root = root;
	}

	public NearbyWords getNw() {
		return nw;
	}

	public void setNw(NearbyWords nw) {
		this.nw = nw;
	}
