...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
			String longS = s+(char)charCode;
			if(!currentList.contains(longS) && 
					(!wordsOnly||dict.isWord(longS)) &&
					!s.equals(longS)) {
				currentList.add(longS);
			}
		}
	}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (retList.size() < numSuggestions && !queue.isEmpty()) {
			String firstStr = queue.removeFirst();
			List<String> tempList = distanceOne(firstStr, true);
