...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			String numToCheckStr = getStringFromFile(textfile, numToCheck);
			long startTimeB = System.nanoTime();
			for (int i=0; i < trials; i++) {
				BasicDocument textBasicDoc = new BasicDocument(numToCheckStr); 
				double fleschScoreBasicDoc = textBasicDoc.getFleschScore();
			}
			long endTimeB = System.nanoTime();
			double estTimeB = ((endTimeB-startTimeB)/100000000.0);
			System.out.print(estTimeB + "\t");
			long startTimeE = System.nanoTime();
			for (int i=0; i < trials; i++) {
				EfficientDocument textEffDoc = new EfficientDocument(numToCheckStr); 
				double fleschScoreEffDoc = textEffDoc.getFleschScore();
			}
			long endTimeE = System.nanoTime();
			double estTimeE = ((endTimeE-startTimeE)/100000000.0);
